...
<template>
  <div class="space-y-6">
    ...
    <div class="bg-white/20 backdrop-blur-lg rounded-xl p-6 shadow-lg text-white">
      <h2 class="text-2xl font-bold mb-2">Prévisions pour {{ weather.name }}</h2>
      <p class="text-white/70">Prochaines 24 heures</p>
    </div>

    <!-- Prévisions -->
    <div v-if="loading" class="text-white text-center py-12">
      Chargement des prévisions...
    </div>
    
    <div v-else-if="error" class="bg-red-500/20 backdrop-blur-lg rounded-xl p-6 text-white text-center">
      {{ error }}
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <div 
        ...
        :key="item.dt"
        class="bg-white/20 backdrop-blur-lg rounded-xl p-6 shadow-lg text-white"
      >
        ...
        <div class="space-y-4">
          ...
          <div class="text-sm capitalize text-center">
