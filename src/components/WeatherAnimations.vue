...
const PARTICLE_CONFIG = {
  rain: { count: 80, speed: { min: 0.8, max: 1.5 }, opacity: { min: 0.1, max: 0.3 } },
  snow: { count: 40, speed: { min: 2, max: 4 }, opacity: { min: 0.2, max: 0.6 } },
...
function createRainDrop() {
  ...
  drop.style.animationDuration = `${Math.random() * (config.speed.max - config.speed.min) + config.speed.min}s`;
  drop.style.opacity = `${Math.random() * (config.opacity.max - config.opacity.min) + config.opacity.min}`;
...
function createLightning() {
  ...
  gsap.timeline()
    .to(lightning, { opacity: PARTICLE_CONFIG.thunder.intensity, duration: 0.05 })
...
function createCloud() {
  ...
  const top = Math.random() * (CLOUD_CONFIG.spacing.max - CLOUD_CONFIG.spacing.min) + CLOUD_CONFIG.spacing.min;
...
function animateCloud(cloud: HTMLElement) {
  const duration = Math.random() * (CLOUD_CONFIG.duration.max - CLOUD_CONFIG.duration.min) + CLOUD_CONFIG.duration.min;
  ...
  gsap.to(cloud, {
    opacity: cloud.style.opacity === "0" ? 0.15 : parseFloat(cloud.style.opacity) * 0.7,
...
function createSnowFlake() {
  ...
  flake.style.animationDuration = `${Math.random() * (config.speed.max - config.speed.min) + config.speed.min}s`;
  flake.style.opacity = `${Math.random() * (config.opacity.max - config.opacity.min) + config.opacity.min}`;
...
function addCloud() {
  if (!animationContainer.value || clouds.value.length >= CLOUD_CONFIG.maxClouds) return;
...
function addWeatherParticles() {
  ...
  if (type.includes("rain") || type.includes("thunder")) {
    ...
    if (type.includes("thunder") && Math.random() < PARTICLE_CONFIG.thunder.lightningChance) {
...
function updateClouds() {
  if (!animationContainer.value) return;
  if (clouds.value.length < CLOUD_CONFIG.maxClouds && Math.random() < CLOUD_CONFIG.spawnRate) {
    addCloud();
  }
}
