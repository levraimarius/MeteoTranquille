...
<template>
  <div class="space-y-6">
    ...
    <div class="bg-white/20 backdrop-blur-lg rounded-xl p-6 shadow-lg text-white">
        ...
        <div class="space-y-2">
          ...
          <div class="flex space-x-4">
            <div>
              ...
              <div class="text-xl">{{ Math.round(weather.main.temp_min) }}°C</div>
            </div>
            <div>
              <div class="text-sm opacity-75">Max</div>
              <div class="text-xl">{{ Math.round(weather.main.temp_max) }}°C</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="bg-white/20 backdrop-blur-lg rounded-xl p-6 shadow-lg text-white">
      <h3 class="text-xl font-semibold mb-4">Description détaillée</h3>
      <div class="space-y-4">
        <p class="text-lg">
          {{ weather.weather[0].description.charAt(0).toUpperCase() + weather.weather[0].description.slice(1) }}
          avec une température de {{ Math.round(weather.main.temp) }}°C.
          Le vent souffle à {{ Math.round(weather.wind.speed * 3.6) }} km/h 
          en direction du {{ formatWindDirection(weather.wind.deg) }}.
        </p>
        <p>
          La visibilité est de {{ formatVisibility(weather.visibility) }} 
          avec une couverture nuageuse de {{ weather.clouds.all }}%.
        </p>
      </div>
    </div>
  </div>
